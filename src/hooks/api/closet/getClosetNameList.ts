...
export async function getClosetNameListAPICall(token: string): Promise<Closet[] | null> {
  try {
    const response = await axios.get(`${process.env.REACT_APP_API_URL}/closet`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (response.status === 200) {
      enqueueSnackbar('옷장 이름 리스트를 가져왔습니다.', { variant: 'success' });
      return response.data.length === 0 ? [] : response.data.map((acloset: Closet) => acloset.name);
    }
  } catch (err) {
